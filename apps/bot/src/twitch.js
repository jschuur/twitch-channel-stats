...
export function authenticateTwitch() {
    ...
    if (refreshToken)
        return new RefreshingAuthProvider({ clientId, clientSecret }, { accessToken, refreshToken, expiresIn: 0, obtainmentTimestamp: 0 });
    else
        return new StaticAuthProvider(clientId, accessToken);
}
export async function connectChat(authProvider, channels) {
    let spinner;
    let chatClient;
    try {
        ...
        for (const channel of channels.split(','))
            await saveEvents({ type: 'channel_join', channel });
    }
    catch (error) {
        spinner?.fail(`Failed to connect to chat (${pc.dim(error.message)})`);
        process.exit(1);
    }
    return chatClient;
}
