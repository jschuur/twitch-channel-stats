...
export async function saveEvents(payload) {
...
export function startupMessages() {
    ...
    envVars.forEach(([envVar, label, sliceLength]) => {
        console.log(`${label.padEnd(labelLength)} ${process.env[envVar]
            ? `${pc.green(process.env[envVar]?.slice(0, sliceLength))}...`
            : pc.red('Not set')}`);
    });
    if (boolean(process.env.DISABLE_EVENT_SAVING))
